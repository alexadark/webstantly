...
const blogTemplate = path.resolve(`./src/templates/posts/archivePosts.js`)
...
module.exports = async ({ actions, graphql }) => {
  ...
  const fetchPosts = async variables => {
    ...
    return await graphql(GET_POSTS, variables).then(({ data }) => {
       ...
       * @type {string}
       */
      const blogPagePath = !variables.after ? `blog/` : `blog/${pageNumber + 1}`
   ...
   */
  await fetchPosts({ first: 8, after: null }).then(allPosts => {
    /**
